 ...
 */
	...
	CASE_0F_B(0x09)												/* WBINVD */
		if (CPU_ArchitectureType<CPU_ARCHTYPE_486OLDSLOW) return PrefixOutcome { .type = IllegalOpCode };
		if (cpu.pmode && cpu.cpl) EXCEPTION(EXCEPTION_GP);
		break;
	...
	CASE_0F_B(0x31)												/* RDTSC */
		{
			if (CPU_ArchitectureType<CPU_ARCHTYPE_PENTIUMSLOW) return PrefixOutcome { .type = IllegalOpCode };
	...
	CASE_0F_B(0xa2)												/* CPUID */
		if (!CPU_CPUID()) return PrefixOutcome { .type = IllegalOpCode };
		break;
