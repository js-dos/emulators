 ...
 */
	...
	CASE_W(0x63)												/* ARPL Ew,Rw */
		{
			if ((reg_flags & FLAG_VM) || (!cpu.pmode)) return PrefixOutcome { .type = IllegalOpCode };
			GetRMrw;
			if (rm >= 0xc0 ) {
	...
	CASE_W(0x8c)												/* Mov Ew,Sw */
		{
			...
			default:
				...
				return PrefixOutcome { .type = IllegalOpCode };
			}
	...
	CASE_W(0x9a)												/* CALL Ap */
		{ 
			...
			if (GETFLAG(TF)) {	
				...
				return PrefixOutcome { .type = CbRet, .ret = CBRET_NONE };
			}
#endif
			return PrefixOutcome { .type = Continue };
		}
	...
	CASE_W(0x9d)												/* POPF */
		...
		if (GETFLAG(TF)) {	
			cpudecoder=CPU_TRAP_DECODER;
                        return PrefixOutcome { .type = DecodeEnd };
		}
#endif
#if	CPU_PIC_CHECK
		if (GETFLAG(IF) && PIC_IRQCheck)  return PrefixOutcome { .type = DecodeEnd };
#endif
		break;
	CASE_B(0x9e)												/* SAHF */
		SETFLAGSb(reg_ah);
	...
	CASE_W(0xc4)												/* LES */
		{	
			GetRMrw;
			if (rm >= 0xc0) return PrefixOutcome { .type = IllegalOpCode };
			GetEAa;
...
#endif
#if CPU_PIC_CHECK
			if (GETFLAG(IF) && PIC_IRQCheck) return PrefixOutcome { .type = CbRet, .ret = CBRET_NONE };
#endif
			return PrefixOutcome { .type = Continue };
		}
	...
	CASE_B(0xf4)												/* HLT */
		...
		FillFlags();
		CPU_HLT(GETIP);
		return PrefixOutcome { .type = CbRet, .ret = CBRET_NONE };		//Needs to return for hlt cpu core
	CASE_B(0xf5)												/* CMC */
		FillFlags();
			...
			case 0x07:										/* CallBack */
				{
					Bitu cb=Fetchw();
					FillFlags();SAVEIP;
					return PrefixOutcome { .type = CbRet, .ret = static_cast<Bits>(cb) };
				}
			default:
